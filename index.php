<?php
// https://netparadis.com
error_reporting(0);
set_time_limit(-1);
header("HTTP/1.0 200 OK");
date_default_timezone_set('Asia/Tehran');
//--------[Your Config]--------//
$Dev = [10999,12225]; // آیدی عددی ادمین را از بات @userinfobot بگیرید
$logs = -1001220562710;
$channel = "@netparadis"; // ربات را ادمین کانال کنید
//-----------------------------//
define('API_KEY','توکن'); // توکن ربات
//------------------------------------------------------------------------------
function netparadis($method, $datas = []){
...
function EditKeyboard($chat_id, $message_id, $keyboard){
	netparadis('EditMessageReplyMarkup',[
    'chat_id'=>$chat_id,
    'message_id'=>$message_id,
	'reply_markup'=>$keyboard
    ]);
...
function DeleteMessage($chatid, $massege_id){
	netparadis('DeleteMessage',[
    'chat_id'=>$chatid,
    'message_id'=>$massege_id
    ]);
}
function Kick($chatid, $fromid){
	netparadis('KickChatMember',[
    'chat_id'=>$chatid,
    'user_id'=>$fromid
    ]);
}
function Admin($chat_id, $from_id){
	global $Dev;
	$get = netparadis('GetChatMember',['chat_id'=> $chat_id, 'user_id'=> $from_id]);
...
if($tc == 'private'){
	if(preg_match('/^\/(start)$/i',$text)){
		...
		SendMessage($logs,"■  کاربر [$first_name](tg://user?id=$from_id) ربات ادد بزن را استارت کرد.", 'MarkDown');
	}
	elseif($rank == 'left'){
	...
	elseif($text == "🥒 نصب رایگان"){
	    $BotID = netparadis('GetMe',[])->result->username;
...
elseif($tc == 'supergroup'){
	...
	if(preg_match('/^\/?(add|نصب)$/ui', $text, $match) and $status == true){
		if(!is_dir("data/$chat_id")){
			...
			$first_name = MarkDown($first_name);
			SendMessage($logs,"■ گروه \[$title] توسط [$first_name](tg://user?id=$from_id) نصب گردید.", 'MarkDown');
		}else{
	...
	elseif(preg_match('/^\/?(panel|پنل)$/ui', $text) and $status == true){
		if(isset($data)){
			...
			$membersgp = netparadis('getChatMembersCount',['chat_id'=> $chat_id])->result;
	...
	elseif(preg_match('/^\/?(up|معاف) (\d+)$/ui', $text, $match) and $status == true){
		if(isset($data)){
			if(!in_array($match[2], $data['whitelist'])){
				...
				file_put_contents("data/$chat_id/setting.json",json_encode($data));
				$name = MarkDown(netparadis('getChat',['chat_id'=> $match[2]])->result->first_name);
	...
	elseif(preg_match('/^\/?(up|معاف)$/ui', $text) and $status == true){
		if(isset($data) and isset($reply_id)){
			if(!in_array($reply_id, $data['whitelist'])){
				...
				file_put_contents("data/$chat_id/setting.json",json_encode($data));
				$name = MarkDown(netparadis('getChat',['chat_id'=> $reply_id])->result->first_name);
		...
		if($status == true){
		    if(empty($data['whitelist']) === false){
				...
				foreach($list as $value){
					$first_name = netparadis('getChat',['chat_id'=> $value])->result->first_name;
					$name = MarkDown($first_name);
				...
				netparadis('AnswerCallbackQuery',[
	...
	elseif($Data == "help"){
		if($status == true){
			EditMessageText($chat_id, $message_id, "📯 راهنمای ربات ادد بزن به شرح ذیل می باشد :\n\n*●* نصب  |  add/\n*-|* نصب ربات در گروه شما\n\n*●* پنل  |  panel/\n*-|* مشاهده پنل تنظیمات ربات در گروه شما\n\n*●* معاف  |  up/  (ریپلی | آیدی عددی)\n*-|* معاف کردن یک کاربر از ادد کردن اجباری\n\n*●* لغو معاف  |  unup/  (ریپلی | آیدی عددی)\n*-|* لغو معافیت یک کاربر از ادد کردن اجباری\n\n■ *Developer :* @Netparadis\n■ *Out Channel :* @Netparadis", 'MarkDown', json_encode(['inline_keyboard'=>[ [['text'=>"🔙 برگشت",'callback_data'=>"panel"]] ]]));
		}
	}
	elseif($Data == "close"){
		if($status == true){
